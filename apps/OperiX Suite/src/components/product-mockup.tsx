...
export function DashboardMockup({ variant = "suite" }: { variant?: "suite" | "invoice" | "hr" | "booking" | "desk" | "control" }) {
  ...
  return (
    <div className={`dashboard-mockup dashboard-${variant}`} aria-label={`${variant} dashboard preview`}>
      ...
      <div className="mock-main">
        <header>
          <div>
            <span>{isHR ? "People overview" : isDesk ? "Workspace overview" : isBooking ? "Booking overview" : isControl ? "Suite overview" : variant === "invoice" ? "Financial overview" : "Business overview"}</span>
            <strong>{isHR ? "Your team, at a glance" : isDesk ? "Floor plan at a glance" : isBooking ? "Today's appointments" : isControl ? "Good morning, Sam" : "Good morning"}</strong>
          </div>
          <div className="mock-avatar"><UserRound aria-hidden="true" /></div>
        </header>
        <div className="mock-stats">
          <div><span>{isHR ? "Employees" : isDesk ? "Desks" : isBooking ? "Appointments" : isControl ? "Revenue" : "Revenue"}</span><strong>{isHR ? "Team" : isDesk ? "42" : isBooking ? "18" : isControl ? "€128,430" : "€24,850"}</strong><small>{isControl ? "↑ 12.4% vs last 30 days" : "Current period"}</small></div>
          <div><span>{isHR ? "Attendance" : isDesk ? "Available" : isBooking ? "Confirmed" : isControl ? "Unpaid invoices" : "Outstanding"}</span><strong>{isHR ? "On track" : isDesk ? "16" : isBooking ? "14" : isControl ? "18" : "€4,280"}</strong><small>{isControl ? "↓ 10.0% vs last 30 days" : "Updated today"}</small></div>
          <div><span>{isHR ? "Leave" : isDesk ? "Rooms" : isBooking ? "Open slots" : isControl ? "Appointments" : "Invoices"}</span><strong>{isHR ? "Overview" : isDesk ? "8" : isBooking ? "6" : isControl ? "42" : "128"}</strong><small>{isControl ? "↑ 16.7% vs last 30 days" : "Live workspace"}</small></div>
        </div>
...
export function PhoneMockup({ variant = "invoice" }: { variant?: "invoice" | "hr" | "booking" | "desk" | "control" }) {
  const isHR = variant === "hr";
  return (
