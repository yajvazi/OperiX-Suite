...
const siteOrigin = process.env.NEXT_PUBLIC_SITE_URL ?? "https://operixsuite.com";
...
export async function POST(request: NextRequest) {
  ...
  if (!verifyPreviewCredentials(username, password)) {
    const loginUrl = new URL("/preview-login", siteOrigin);
    loginUrl.searchParams.set("error", "invalid");
    loginUrl.searchParams.set("next", nextPath);
    return NextResponse.redirect(loginUrl, 303);
  }

  const response = NextResponse.redirect(new URL(nextPath, siteOrigin), 303);
