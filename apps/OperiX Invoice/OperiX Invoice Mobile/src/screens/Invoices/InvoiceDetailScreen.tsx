...
import { ArrowLeft, Edit, Share2, FileText, Check, Trash2, Eye, RefreshCw, Mail, Zap, CreditCard, Download, Printer, Receipt, X } from 'lucide-react-native';
...
export function InvoiceDetailScreen({ navigation, route }: InvoiceDetailScreenProps) {
    ...
    const handlePrintWithTemplate = async (templateToUse: TemplateType) => {
        ...
        try {
            ...
            const result = await printPdf(printData, templateToUse);
                    ...
                    <View style={[styles.summarySection, { borderTopColor: borderColor }]}>
                        ...
                        {Number(invoice.discount_amount) > 0 && (
                            <View style={styles.summaryRow}>
                                ...
                                <Text style={[styles.summaryValue, { color: '#12B76A' }]}>-{formatCurrency(Number(invoice.discount_amount))}</Text>
                            </View>
                        )}
            ...
            <Modal visible={showPreview} animationType="slide">
                <View style={{ flex: 1, backgroundColor: bgColor }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingTop: 56, paddingBottom: 16 }}>
                        ...
                        <Text style={{ fontSize: 18, fontWeight: '600', color: textColor }}>Invoice Preview</Text>
