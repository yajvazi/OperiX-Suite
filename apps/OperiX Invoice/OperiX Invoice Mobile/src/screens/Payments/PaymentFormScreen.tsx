...
export function PaymentFormScreen({ navigation, route }: any) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isEditing) {
                ...
                if (error) throw error;
                try {
                    await shareTransactionPdf({ title: 'Income payment', number: formData.payment_number, amount: Number(formData.amount), date: formData.payment_date, method: formData.payment_method, counterparty: selectedClient.name, reference: formData.bank_reference });
                } catch (pdfError) {
    ...
    const paymentMethods = [
        { key: 'cash', label: 'Cash', icon: Banknote, color: '#12B76A' },
        { key: 'bank', label: 'Bankë', icon: Building, color: '#3388FF' },
        { key: 'card', label: 'Kartelë', icon: CreditCard, color: '#3388FF' },
    ];
