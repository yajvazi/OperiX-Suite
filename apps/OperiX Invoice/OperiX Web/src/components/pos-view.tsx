    ...
    <div className="mx-auto max-w-[1800px]">
      ...
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1.08fr)_minmax(460px,.92fr)]">
        ...
        <section className="card min-w-0 overflow-hidden xl:sticky xl:top-20">
          ...
          <div className="border-b border-[#edf0f4] p-4"><p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-[#667085]">Payment method</p><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{paymentOptions.map((option) => {const Icon = option.icon; const active = payment === option.value; return <button key={option.value} type="button" aria-pressed={active} className={`btn pos-payment-option h-10 px-2 text-xs ${active ? "pos-payment-active" : ""}`} onClick={() => setPayment(option.value)}><Icon size={15}/>{option.label}</button>;})}</div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="field"><span>Cash received</span><div className="flex gap-2"><input className="input min-w-0" type="number" min="0" step="1" value={cashReceived.toFixed(2)} onChange={(event) => {const value=Number(event.target.value);setCashReceived(Number.isFinite(value) ? Math.max(0, Math.round(value)) : 0);}} disabled={payment !== "cash"}/><button type="button" className="btn shrink-0 px-3 text-xs" onClick={() => setCashReceived(Math.round(totals.total))} disabled={payment !== "cash" || !cart.length}>Paid in full</button></div></label><div className="field"><span>{amountDue > 0 ? "Amount due" : "Change"}</span><strong className={`input ${amountDue > 0 ? "bg-[#fff8eb] text-[#b54708]" : "bg-[#ecfdf3] text-[#087443]"}`}>{money(amountDue || change, currency)}</strong></div></div></div>
