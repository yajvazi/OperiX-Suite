...
import { ArrowLeft, CreditCard, Download, Edit3, Mail, MoreHorizontal, Printer, ReceiptText } from "lucide-react";
...
export function InvoiceDetail({id, invoiceNumber}:{id?:string; invoiceNumber?:string}){
  ...
  async function createPaymentLink(){if(!invoice)return;setPaymentBusy(true);setError("");const response=await fetch("/api/payments/create-link",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({invoiceId:invoice.id})});const result=await response.json().catch(()=>({}));if(!response.ok){setError(result.error||"Unable to create an online payment link.");setPaymentBusy(false);return;}setPaymentLink(String(result.url||""));setPaymentBusy(false);}
  ...
  return <main className="invoice-workspace">
    <header className="invoice-commandbar no-print">
      ...
      <div className="invoice-command-actions">
        ...
        <button className="btn" onClick={createPaymentLink} disabled={paymentBusy}><CreditCard size={16}/>{paymentBusy?"Creating…":"Payment link"}</button>
    ...
    {paymentLink?<p className="no-print mx-auto mb-4 max-w-[1400px] rounded bg-[#e9f9f0] p-3 text-xs text-[#067647]">Payment link ready: <a className="underline" href={paymentLink} target="_blank" rel="noreferrer">Open Stripe checkout</a></p>:null}
