...
export function Brand({ dark = false, compact = false }: { dark?: boolean; compact?: boolean }) {
  ...
  return <Link href="/dashboard" className="inline-flex items-center gap-3" aria-label="OperiX Invoice dashboard">
    {compact ? <Image src={`/operix-x-${dark ? "blue" : "white"}.svg`} width={48} height={32} alt="" aria-hidden="true" priority /> : <Image src={source} width={140} height={70} alt="OperiX Invoice" aria-hidden="true" priority />}
  </Link>;
}
